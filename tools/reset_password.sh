...
elif [[ $(mysql --version) =~ "10.2" ]] || [[ $(mysql --version) =~ "10.3" ]]
then
	/usr/bin/mysql -u root mysql << EOF
...
then
	...
	then
		...
		echo -e "mariadb administrator username:root
mariadb administrator password:$mysql_root_password\n\n\n\n" >  /credentials/password.txt
	fi
